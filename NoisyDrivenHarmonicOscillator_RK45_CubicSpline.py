...
def f(t, S):
    ...
    dSdt[1] = - F(t) / m - 2 * r * S[1] - (omega ** 2) * S[0]  # Derivative of velocity
...
def noise(gamma, T, t0, tf, h):
    ...
    # for i in range(len(time_matrix)):
        # for j in range(len(time_matrix)):
            ...
            # if (i == j):
                # correlation_matrix[i][j] = 2 * gamma * 1.380649 * 10 ** (-23) * T

    # L = np.linalg.cholesky(correlation_matrix)
    noise = 2 * gamma * 1.380649 * 10 ** (-23) * T * np.random.normal(loc = 0, scale = sqrt(2 * gamma * 1.380649 * 10 ** (-23) * T), size = int((tf - t0) / h))
    return noise

# Global variables
global r, omega, error_m, omega_f, F0, h_interpolate
...
T = float(input("Temperature:")) # Temperature
t0 = 0.0  # Start time
tf = 5.0  # End time
...
t_values_splined = np.linspace(t0, tf, int((tf - t0) / h_interpolate))  # Generate dense time values for spline
...
# Adding noise
noise = noise(gamma, T, t0, tf, h_interpolate)
x_values_splined_noise = np.zeros_like(x_values_splined)
for i in range(len(x_values_splined)):
    x_values_splined_noise[i] = x_values_splined[i] + noise[i] * sqrt(h_interpolate)
...
freq = (1 / (h_interpolate * n)) * np.arange(n)  # Frequency array
...
plt.subplot(9, 1, 1)  # Create a subplot for position and velocity
...
plt.subplot(9, 1, 3)  # Create a subplot for the cubic spline
...
plt.subplot(9, 1, 5)  # Create a subplot for the FFT
...
plt.subplot(9, 1, 7)  # Create a subplot for position and velocity
plt.plot(t_values_splined, noise)  # Plot position
plt.xlabel('Time')  # X-axis label
plt.ylabel('Noise')  # Y-axis label
plt.title('Noise against Time')  # Plot title
plt.grid(True)  # Show grid

plt.subplot(9, 1, 9)  # Create a subplot for position and velocity
plt.plot(t_values_splined, x_values_splined_noise)  # Plot position
plt.xlabel('Time')  # X-axis label
plt.ylabel('Position')  # Y-axis label
plt.title('Noisy Position against Time')  # Plot title
plt.grid(True)  # Show grid
plt.savefig('moving_mirror_with_noise_rk45_and_cubic_spline.png')  # Save the plot
